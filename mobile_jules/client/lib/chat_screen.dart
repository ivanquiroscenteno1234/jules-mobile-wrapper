...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  void _showFullDiff(String patch) {
    showModalBottomSheet(
      ...
      builder: (context) {
        // ⚡ Bolt: Split the patch string once and use ListView.builder to lazily
        // construct widgets, preventing UI thread blockage and memory spikes for large diffs.
            ...
            children: [
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) =>
                      _buildDiffLine(diffLines[index]),
                ),
              ),
            ],
          ),
  ...
  Widget _buildCompletedBadge(ChatMessage message) {
    ...
    print(
      'DEBUG _buildCompletedBadge: hasPR=$hasPR, hasPatch=$hasPatch, sessionId=$sessionId, julesUrl=$julesUrl',
    );
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        onPressed: _isPublishing
                            ? null
                            : () =>
                                  _createGitHubPR(sessionId, branchOnly: true),
                        icon: _isPublishing
                            ? const SizedBox(
                                width: 16,
                                height: 16,
                    ...
                    Expanded(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: _isPublishing
                            ? null
                            : () =>
                                  _createGitHubPR(sessionId, branchOnly: false),
                        icon: _isPublishing
                            ? const SizedBox(
                                width: 16,
                                height: 16,
                        ...
                        label: Text(
                          _isPublishing ? 'Creating...' : 'Create PR',
                        ),
                        style: ElevatedButton.styleFrom(
