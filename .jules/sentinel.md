...
## 2025-02-28 - [Fix Information Exposure in GitHub Repo Creation]
**Vulnerability:** The `/github/repos` endpoint raised a 500 `HTTPException` with the detail set to `str(e)` when an unexpected `httpx.HTTPStatusError` occurred. This leaked internal HTTP response details (such as the target API URL and error context) to the client, leading to Information Exposure.
**Learning:** Returning `str(e)` directly in an `HTTPException` is dangerous because the string representation of external HTTP client errors (like `httpx.HTTPStatusError`) often includes the full request URL, method, and potentially query parameters. This can expose backend infrastructure details to an attacker.
**Prevention:** Catch external API exceptions, log their raw string representations server-side securely (e.g., using `print` or a logging framework), and return a sanitized, generic error message (e.g., 'GitHub API request failed') to the client via the `HTTPException`.
